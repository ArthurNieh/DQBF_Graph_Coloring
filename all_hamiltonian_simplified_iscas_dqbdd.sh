...
dir_path="iscas89/result/hamiltonian_simplified_dqbdd/"
...
for (( FF_num=3; FF_num<=3; FF_num=FF_num+1 )); do
	...
	for bench in "${sorted_benches[@]}"; do	
		...
		file_name="${dir_path}/hamiltonian_${ins}_FF${FF_num}"
		...
		./my_hamiltonian_simplified_iscas_dqbdd.sh "$ins" "$FF_num" &>> "$file_name"
