...
solver=$4
if [ -z "$instance" ]; then
    echo "Usage: $0 <instance> <k_num> [trial] [solver]"
    exit 1
fi
if [ -z "$k_num" ]; then
    echo "Usage: $0 <instance> <k_num> [trial] [solver]"
...
cd ../
python3 blif_gen_random_graph_clique.py -n "$instance" -k "$k_num" -t "$trial"
...
cd ../abc/dqbf
python3 convert_to_cnf_clique.py \
    "../../random_graph/sample/random_graph_clique_n${instance}_k${k_num}_trial${trial}.blif"
