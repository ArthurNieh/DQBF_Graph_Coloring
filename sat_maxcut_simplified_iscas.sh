...
start1=`date +%s.%N`
timeout 1h python3 explicit_gen_iscas.py "${instance}_simplified" -m
# this will generate iscas_graph.txt
end_p=`date +%s.%N`
# Run the Biqbin solver
echo -e "\nRun the Biqbin solver"
cd ../bin/
timeout 1h ./biqbin ../iscas89/sample/iscas_graph.txt params
...
echo -e "\nRuntime for Biqbin solver was $runtime_solver seconds.\n"
