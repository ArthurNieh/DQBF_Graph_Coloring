...
mkdir -p iscas89/result/simplified/
rm iscas89/benchmarks/*simplified.*

for (( FF_num=12; FF_num<=12; FF_num=FF_num+1 )); do
	...
	# Create an array of sorted benchmarks by numeric instance number
		...
		for bench in "${sorted_benches[@]}"; do	
			...
			output="iscas89/result/simplified/${ins}_C${i}_FF${FF_num}"
			...
			./my_simplified_iscas.sh "$ins" "$i" "$FF_num" &>> "$output"
