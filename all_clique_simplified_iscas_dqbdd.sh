...
mkdir -p iscas89/result/clique_simplified_dqbdd/
...
for (( FF_num=3; FF_num<=3; FF_num=FF_num+1 )); do
	...
	# Create an array of sorted benchmarks by numeric instance number
		...
		for bench in "${sorted_benches[@]}"; do	
			...
			output="iscas89/result/clique_simplified_dqbdd/${ins}_Clique${i}_FF${FF_num}"
			...
			./my_clique_simplified_iscas_dqbdd.sh "$ins" "$i" "$FF_num" &>> "$output"
