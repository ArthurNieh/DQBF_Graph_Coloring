...
if [ -z "$blif_file" ] || [ -z "$FF_tokeep" ]; then
    echo "Usage: $0 <blif_file> <FF_tokeep>"
    exit 1
fi
...
SAMPLE_DIR="$SMV_DIR/sample/hamiltonian/pedant"
...
SAMPLE_BLIF="$SAMPLE_DIR/${blif_file}_FF${FF_tokeep}_hamiltonian_sample.blif"
...
######################################################
# Step 3: Generate Hamiltonian SMV/BLIF
######################################################

echo "######################################################"
echo "Generate hamiltonian BLIF"
start2=$(date +%s.%N)

python3 "$SMV_DIR/blif_gen_smv_hamiltonian.py" \
    -i "$COMB_BLIF" \
    -o "$SAMPLE_BLIF" \
    --abc "$ABC_OUTPUT_FILE"
...
if [ ! -f "$SAMPLE_BLIF" ]; then
    echo "ERROR: Hamiltonian BLIF not found: $SAMPLE_BLIF"
    exit 1
fi

start3=$(date +%s.%N)

python3 "$ABC_DQBF_DIR/smv_convert_to_cnf_hamiltonian.py" \
    -i "$SAMPLE_BLIF" \
    -c "$CNF_OUTPUT_FILE" \
...
runtime_hamiltonian=$(echo "$start3 - $start2" | bc -l)
runtime_cnf=$(echo "$start4 - $start3" | bc -l)
runtime_pedant=$(echo "$end - $start4" | bc -l)

echo
echo "================ Runtime Summary ================"
echo "Hamiltonian generation : $runtime_hamiltonian s"
